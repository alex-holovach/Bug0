...
const treeVariants = cva(
    'group hover:bg-accent/70 transition-all duration-200 rounded-lg'
)

const selectedTreeVariants = cva(
    'bg-primary/10 text-foreground font-medium border-l-2 border-l-primary shadow-sm dark:bg-primary/20 dark:text-primary-foreground'
)

const dragOverVariants = cva(
    'bg-primary/10 text-foreground dark:bg-primary/20 dark:text-primary-foreground'
)
    ...
    (
        ...
        return (
            <div
                ref={ref}
                className={cn(
                    'ml-5 flex text-left items-center py-2 px-2 cursor-pointer rounded-lg',
                    treeVariants(),
                    className,
                    selectedItemId === item.id && selectedTreeVariants(),
                    isDragOver && dragOverVariants(),
...
>(({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header>
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                'flex flex-1 w-full items-center py-2 px-2 transition-all first:[&[data-state=open]>svg]:rotate-90 rounded-lg',
                className
            )}
            {...props}
        >
...
const TreeIcon = ({
    ...
    return Icon ? (
        <Icon className={cn(
            ...
            isSelected ? "text-foreground dark:text-primary-foreground" : "text-muted-foreground"
        )} />
    ) : (
        <></>
    )
